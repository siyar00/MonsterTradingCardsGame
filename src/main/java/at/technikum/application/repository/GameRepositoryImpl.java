...
public class GameRepositoryImpl implements GameRepository {
    ...
    public Response startBattle(String username) {
        try (Connection connection = connector.getConnection()) {
            ...
            try {
                ...
                Response deck = getDeck(connection, rs.getInt("user_id"), username);
    ...
    private void fight(String user1, String user2) {
        List<CardRec> firstPlayerDeck = new ArrayList<>(GameRepositoryImpl.USER_DECK_LISTS.get(user1));
        List<CardRec> secondPlayerDeck = new ArrayList<>(GameRepositoryImpl.USER_DECK_LISTS.get(user2));
        GAME result = null;
    ...
    private void logger(CardRec firstPlayerCard, CardRec secondPlayerCard, GAME game){
        battleLog.append(firstPlayerCard.USER()).append(": ").append(firstPlayerCard.name()).append(" (")
                .append(firstPlayerCard.damage()).append(" Damage) VS ").append(secondPlayerCard.USER()).append(": ")
                .append(secondPlayerCard.name()).append(" (").append(secondPlayerCard.damage()).append(" Damage) => ");
        if(GAME.WIN_PLAYER1 == game) battleLog.append(firstPlayerCard.name()).append(" defeats ").append(secondPlayerCard.name()).append("\n");
        else if(GAME.WIN_PLAYER2 == game) battleLog.append(secondPlayerCard.name()).append(" defeats ").append(firstPlayerCard.name()).append("\n");
    ...
    private void logger(CardRec firstPlayerCard, CardRec secondPlayerCard, Double damage, Double damage1, GAME game){
        ...
        else if(GAME.WIN_PLAYER2 == game) battleLog.append(secondPlayerCard.name()).append(" wins\n");
        else battleLog.append("Draw (no action)\n");
    }
